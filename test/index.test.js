...
describe('collect', function () {
  it('should collect the iterator asynchronously', async () => {
    async function * generate(i = 10) {
      while (i--) {
        await wait(i);
        yield i;
      }
    }
    assume(await collect(generate())).eqls([9, 8, 7, 6, 5, 4, 3, 2, 1, 0]);
  });

  it('should collect the iterator synchronously', async () => {
    function * generate(i = 10) {
      while (i--) {
        yield i;
      }
    }
